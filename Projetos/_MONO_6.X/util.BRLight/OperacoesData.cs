...
namespace util.BRLight {
    ...
    public static class OperacoesData
    {
        ...
        public static string DataPorExtenso(string data)
        {
            ...
            string[] mes_por_extenso = { "", "Janeiro", "Fevereiro", "Março", "Abril", "Maio", "Junho", "Julho", "Agosto", "Setembro", "Outubro", "Novembro", "Dezembro" };
            var day = aDate[0];
            var month = aDate[1];
            var year = aDate[2];

            var data_extenso = day + " de " + (month.Length > 1 && month[0] == '0'  ? mes_por_extenso[int.Parse(month)] : mes_por_extenso[int.Parse(char.ToString(month[1]))]) + " de " + year;
