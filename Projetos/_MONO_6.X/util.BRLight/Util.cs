...
namespace util.BRLight
{
    public static class Util
    {
        ...
        public static string GetUserIp()
        {
            ...
            try {
                ...
                if (string.IsNullOrEmpty(ipAddress)) {
                    ipAddress = HttpContext.Current.Request.ServerVariables["X-Forwarded-For"];
                    if (string.IsNullOrEmpty(ipAddress))
                    {
        ...
        public static string GetServerPort()
        {
            try
            {
                var porta = Variables("Host");

                if (string.IsNullOrEmpty(porta))
                {
                    porta = Variables("SERVER_PORT");
                    
                   if (string.IsNullOrEmpty(porta))
                   {
                        porta = HttpContext.Current.Request.Url.Port.ToString();;
                   }
                }
                return porta;
            }
        ...
        // NOTE: Este método é exclusivo para o SINJ e não existe na versão mais
        // recente deste biblioteca. By Questor
        ...
        public static string GetPadrao()
        {
            try
            {
                var sPadrao = HttpContext.Current.Application["Padrao"];
                if (sPadrao == null)
                {
                    sPadrao = Config.ValorChave("Padrao", true);
                    if (Equals(sPadrao, "-1"))
                    {
                        sPadrao = "Criar_Chave_Padrao_No_Config";
                    }
                    if (sPadrao.ToString() == "")
                    {
                        sPadrao = "Chave_Padrao_No_Config_Vazia";
                    }
                    HttpContext.Current.Application["Padrao"] = sPadrao;
                }
                return sPadrao.ToString();
            }
            catch
            {
                return "getPadraoErro";
            }
        }
        ...
        public static string GetAplicacao()
        {
            try
            {
                var sAplicacao = HttpContext.Current.Application["Aplicacao"];
                if (sAplicacao == null)
                {
                    sAplicacao = Config.ValorChave("Aplicacao");
                    if (Equals(sAplicacao, "-1"))
                    {
                        sAplicacao = "Criar_Chave_Aplicacao_No_Config";
                    }
                    if (sAplicacao.ToString() == "")
                    {
                        sAplicacao = "Chave_Aplicacao_No_Config_Vazia";
                    }
                    HttpContext.Current.Application["Aplicacao"] = sAplicacao;
                }
                return sAplicacao.ToString();
            }
            catch
            {
                return "getAplicacaoErro";
            }
        }
