...
namespace util.BRLight
{
    ...
    public static class ValidaDados
    {
        ...
        /// <summary>
        /// Nova validação da nup
        /// </summary>
        /// <returns></returns>
        public static bool ValidaNupTdosAnos (string valor)
        {
            try
            {
                if (!string.IsNullOrEmpty(valor))
                {
                    var index1 = valor.IndexOf('/') + 1;

                    var index2 = valor.IndexOf('-');

                    var indexTotal = index2 - index1;


                    var sAnoProc = valor.Substring(index1, indexTotal);

                    if (sAnoProc.Length == 2)
                    {

                        if (sAnoProc[0] == '0')
                        {
                            sAnoProc = "20" + sAnoProc;
                        }
                        if (sAnoProc[0] > '1')
                        {
                            sAnoProc = "19" + sAnoProc;
                        }

                    }
                    int AnoProc = 0;
                    if (!string.IsNullOrEmpty(sAnoProc))
                    {
                        AnoProc = int.Parse(sAnoProc);
                    }
                    ...
                    var divDado = valor.Substring(valor.Length - 2);
                    valor = valor.Substring(0, valor.Length - 2);
                    tamanho = valor.Length;

                    var Dv1 = calcula1(valor, tamanho, indice, nr_peso, AnoProc);
                    var Dv2 = calcula1(valor + Dv1, tamanho + 1, indice, nr_peso, AnoProc);
                    var Dvs = "" + Dv1 + Dv2 + "";
                    ...
                    if (divDado != "")
                    {
                        ...
                        {
                            var Dv11 = calcula1(valor, tamanho, indice, nr_peso, AnoProc);
                            Dv2 = calcula1(valor + Dv1, tamanho + 1, indice, nr_peso, AnoProc);
                            Dvs = "" + Dv11 + Dv2 + "";
                            if (divDado == Dvs)
                            {
                                return true;
        ...
        public static double calcula1(string valor, int tamanho, ulong indice, ulong peso, int AnoProc)
        {
            double somatoria = 0;
            while (tamanho != 0)
            {
                var sut = valor.Substring(tamanho - 1,1);

                var teste = ulong.Parse(sut);

                somatoria = (teste * indice) + somatoria;
                tamanho = tamanho - 1;
                indice = indice + 1;
                if (indice > peso)
                { indice = 2; }
            ...
            var k = (Math.Floor(somatoria / 11)) * 11;
            var Modval = somatoria - k;
            double Dvcomp = 11 - Modval;
            double Dv = Dvcomp;

            if ((AnoProc > 0 && (Dv == 10)) || (AnoProc > 0 && (Dv == 11)))
            {
                if (Dvcomp == 10) Dv = 0;
                if (Dvcomp == 11) Dv = 1;
