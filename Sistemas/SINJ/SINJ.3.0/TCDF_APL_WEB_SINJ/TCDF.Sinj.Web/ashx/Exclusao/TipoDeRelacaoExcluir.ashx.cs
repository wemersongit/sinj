...
{
    /// <summary>
    /// Summary description for TipoDeRelacaoExcluir
    /// </summary>
    public class TipoDeRelacaoExcluir : IHttpHandler
    {
        ...
        public void ProcessRequest(HttpContext context)
        {
            ...
            ulong id_doc = 0;
            var action = AcoesDoUsuario.sit_exc;
            ...
            try
            {
                if (ulong.TryParse(_id_doc, out id_doc))
                {
                    ...
                    var tipoDeRelacaoOv = new TipoDeRelacaoRN().JsonReg(id_doc);
                    if (new TipoDeRelacaoRN().Excluir(id_doc))
                    {
                        ...
                        try
                        {
                            ...
                            excluidoOv.json_doc_excluido = tipoDeRelacaoOv;
                            excluidoOv.nm_base_excluido = Config.ValorChave("NmBaseTipoDeRelacao", true);
