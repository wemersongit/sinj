...
{
    /// <summary>
    /// Summary description for UsuarioDatatable
    /// </summary>
    public class UsuarioDatatable : IHttpHandler
    {
        ...
        public void ProcessRequest(HttpContext context)
        {
            ...
            var _filtro_st_usuario = context.Request.Params["filtro_st_usuario"];
            ...
            var action = AcoesDoUsuario.usr_pes;
            ...
            try
            {
                ...
                {
                    query = "Upper(document::text) like '%" + _sSearch.ToUpper() + "%'";
                }
                if (!string.IsNullOrEmpty(_filtro_st_usuario))
                {
                    var aux_query = "";
                    foreach (var filtro in _filtro_st_usuario.Split(','))
                    {
                        aux_query += (aux_query != "" ? " or " : "") + "st_usuario=" + filtro;
                    }
                    query += (query != "" ? " and " : "") + "(" + aux_query + ")";
                }
                pesquisa.literal = query;

                json_resultado = new UsuarioRN().JsonReg(pesquisa);
