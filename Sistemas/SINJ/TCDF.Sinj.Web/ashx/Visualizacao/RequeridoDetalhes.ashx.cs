...
{
    /// <summary>
    /// Summary description for RequeridoDetalhes
    /// </summary>
    public class RequeridoDetalhes : IHttpHandler
    {
        ...
        public void ProcessRequest(HttpContext context)
        {
            ...
            var _ch_requerido = context.Request["ch_requerido"];
            ulong id_doc = 0;
            var requeridoRn = new RequeridoRN();
            RequeridoOV requeridoOv = null;
            var action = AcoesDoUsuario.rqi_vis;
            ...
            try
            {
                ...
                else if (!string.IsNullOrEmpty(_ch_requerido))
                {
                    requeridoOv = requeridoRn.Doc(_ch_requerido);
                }
                else
                {
                ...
                if (requeridoOv != null)
                {
                    sRetorno = JSON.Serialize<RequeridoOV>(requeridoOv);
                }
                else
                {
