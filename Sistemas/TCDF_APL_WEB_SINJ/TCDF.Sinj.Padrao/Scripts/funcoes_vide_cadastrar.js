...
function selecionarSemCitacaoNormaAlteradora(){
    if($('#inSemCitacaoNormaAlteradora').is(':checked')){
        ...
        if(isLecoVide(tipoDeRelacao.ch_tipo_relacao)){
            ...
            var linkNormaAlterada = montarLinkNorma(normaAlterada);
            $(selectorInsertBeforeAlterador).before(`<p ch_norma_info="${normaAlterada.ch_norma}"><a href="${linkNormaAlterada}">${tipoDeRelacao.ds_texto_para_alterador} - ${normaAlterada.ds_norma}</a></p>`);
        }
    }
    else{
    ...
    if(isAcrescimoVide(tipoDeRelacao.ch_tipo_relacao)){
        ...
        if(isLecoVide(tipoDeRelacao.ch_tipo_relacao)){
            ...
            if(!IsNotNullOrEmpty(texto)){
                ...
                $elementoAlterado.html(`<button type="button" class="clean" onclick="desfazerAlteracaoDoDispositivoCadastrar('${dispositivoAlterado.linkname}')"><img src="${_urlPadrao}/Imagens/ico_undo_p.png" width="14px" height="14px" /></button><s>${$elementoAlterado.html()}</s>&nbsp;<a class="link_vide" href="${linkNormaAlteradora}">(${dispositivoAlterado.nm_linkName ? dispositivoAlterado.nm_linkName + ' ' : ''}${tipoDeRelacao.ds_texto_para_alterador} pelo(a) ${normaAlteradora.ds_norma})</a>`);
            }
            else{
                $buttonSelected.hide();
            ...
            for(let i in $conteudoArquivoNormaAlteradora){
                if($conteudoArquivoNormaAlteradora[i].outerHTML){
                    if($conteudoArquivoNormaAlteradora[i].getAttribute('linkname') == normaAlteradora.dispositivos[0].linkname){
                        ...
                        let link = montarLinkNorma(normaAlterada);
