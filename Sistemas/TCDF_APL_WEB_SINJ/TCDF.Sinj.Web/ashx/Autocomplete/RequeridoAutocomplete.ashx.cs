...
{
    /// <summary>
    /// Summary description for RequeridoAutocomplete
    /// </summary>
    public class RequeridoAutocomplete : IHttpHandler
    {
        ...
        public void ProcessRequest(HttpContext context)
        {
            ...
            if (!string.IsNullOrEmpty(_texto))
            {
                if (_texto != "...")
                {
                    sQuery = "Upper(nm_requerido) like'%" + _texto.ToUpper() + "%'";
                }
            }

            query.literal = sQuery;
            query.order_by.asc = new[] { "nm_requerido" };
            context.Response.Clear();

            try
            {
                sRetorno = new RequeridoRN().JsonReg(query);
            }
            catch (Exception Ex)
            {
                var retorno = new
