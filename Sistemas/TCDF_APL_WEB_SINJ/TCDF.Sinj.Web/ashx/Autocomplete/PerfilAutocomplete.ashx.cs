...
{
    /// <summary>
    /// Summary description for PerfilAutocomplete
    /// </summary>
    public class PerfilAutocomplete : IHttpHandler
    {
        ...
        public void ProcessRequest(HttpContext context)
        {
            ...
            if (!string.IsNullOrEmpty(_texto))
            {
                if (_texto != "...")
                {
                    sQuery = "Upper(nm_perfil) like'%" + _texto.ToUpper() + "%' or Upper(ch_perfil) like'%" + _texto.ToUpper() + "%'";
                }
            }

            query.literal = sQuery;
            query.order_by.asc = new[] { "nm_perfil" };
            context.Response.Clear();

            try
            {
                sRetorno = new PerfilRN().JsonReg(query);
            }
            catch (Exception Ex)
            {
                var retorno = new
