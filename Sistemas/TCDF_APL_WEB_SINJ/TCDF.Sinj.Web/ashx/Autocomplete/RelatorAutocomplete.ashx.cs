...
{
    /// <summary>
    /// Summary description for RelatorAutocomplete
    /// </summary>
    public class RelatorAutocomplete : IHttpHandler
    {
        ...
        public void ProcessRequest(HttpContext context)
        {
            ...
            if (!string.IsNullOrEmpty(_texto))
            {
                if (_texto != "...")
                {
                    sQuery = "Upper(nm_relator) like'%" + _texto.ToUpper() + "%'";
                }
            }

            query.literal = sQuery;
            query.order_by.asc = new[] { "nm_relator" };
            context.Response.Clear();

            try
            {
                sRetorno = new RelatorRN().JsonReg(query);
            }
            catch (Exception Ex)
            {
                var retorno = new
