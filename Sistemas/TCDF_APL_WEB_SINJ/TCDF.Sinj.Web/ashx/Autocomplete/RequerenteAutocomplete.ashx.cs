...
{
    /// <summary>
    /// Summary description for RequerenteAutocomplete
    /// </summary>
    public class RequerenteAutocomplete : IHttpHandler
    {
        ...
        public void ProcessRequest(HttpContext context)
        {
            ...
            if (!string.IsNullOrEmpty(_texto))
            {
                if (_texto != "...")
                {
                    sQuery = "Upper(nm_requerente) like'%" + _texto.ToUpper() + "%'";
                }
            }

            query.literal = sQuery;
            query.order_by.asc = new[] { "nm_requerente" };
            context.Response.Clear();

            try
            {
                sRetorno = new RequerenteRN().JsonReg(query);
            }
            catch (Exception Ex)
            {
                var retorno = new
