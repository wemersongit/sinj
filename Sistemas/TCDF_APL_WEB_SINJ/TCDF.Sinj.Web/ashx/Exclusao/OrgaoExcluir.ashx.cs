...
{
    /// <summary>
    /// Summary description for OrgaoExcluir
    /// </summary>
    public class OrgaoExcluir : IHttpHandler
    {
        ...
        public void ProcessRequest(HttpContext context)
        {
            ...
            ulong id_doc = 0;
            var action = AcoesDoUsuario.org_exc;
            ...
            try
            {
                if (ulong.TryParse(_id_doc, out id_doc))
                {
                    ...
                    var orgaoOv = new OrgaoRN().JsonReg(id_doc);
                    if (new OrgaoRN().Excluir(id_doc))
                    {
                        sRetorno = "{\"excluido\":true, \"id_doc_success\":"+id_doc+"}";
                        var nm_base = "";
                        try
                        {
                            nm_base = Config.ValorChave("NmBaseOrgao", true);
                            excluidoOv = new ExcluidoOV();
                            excluidoOv.id_doc_excluido = id_doc;
                            excluidoOv.json_doc_excluido = orgaoOv;
                    ...
                    {
                        throw new Exception("Erro ao excluir órgão. id_doc:"  + _id_doc );
                    }
