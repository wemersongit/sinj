...
{
    /// <summary>
    /// Summary description for TipoDePublicacaoExcluir
    /// </summary>
    public class TipoDePublicacaoExcluir : IHttpHandler
    {
        ...
        public void ProcessRequest(HttpContext context)
        {
            ...
            ulong id_doc = 0;
            var action = AcoesDoUsuario.int_exc;
            ...
            try
            {
                if (ulong.TryParse(_id_doc, out id_doc))
                {
                    ...
                    var tipoDePublicacaoOv = new TipoDePublicacaoRN().JsonReg(id_doc);
                    if (new TipoDePublicacaoRN().Excluir(id_doc))
                    {
                        ...
                        try
                        {
                            ...
                            excluidoOv.json_doc_excluido = tipoDePublicacaoOv;
                            excluidoOv.nm_base_excluido = Config.ValorChave("NmBaseTipoDePublicacao", true);
