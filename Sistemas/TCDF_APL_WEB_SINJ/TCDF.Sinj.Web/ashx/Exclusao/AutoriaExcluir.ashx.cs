...
{
    /// <summary>
    /// Summary description for AutoriaExcluir
    /// </summary>
    public class AutoriaExcluir : IHttpHandler
    {
        ...
        public void ProcessRequest(HttpContext context)
        {
            ...
            ulong id_doc = 0;
            var action = AcoesDoUsuario.aut_exc;
            ...
            try
            {
                if (ulong.TryParse(_id_doc, out id_doc))
                {
                    ...
                    var autoriaOv = new AutoriaRN().JsonReg(id_doc);
                    if (new AutoriaRN().Excluir(id_doc))
                    {
                        ...
                        try
                        {
                            ...
                            excluidoOv.json_doc_excluido = autoriaOv;
                            excluidoOv.nm_base_excluido = Config.ValorChave("NmBaseAutoria", true);
