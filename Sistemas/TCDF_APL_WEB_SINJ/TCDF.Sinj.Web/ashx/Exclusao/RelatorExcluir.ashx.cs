...
{
    /// <summary>
    /// Summary description for RelatorExcluir
    /// </summary>
    public class RelatorExcluir : IHttpHandler
    {
        ...
        public void ProcessRequest(HttpContext context)
        {
            ...
            ulong id_doc = 0;
            var action = AcoesDoUsuario.rel_exc;
            ...
            try
            {
                if (ulong.TryParse(_id_doc, out id_doc))
                {
                    ...
                    var relatorOv = new RelatorRN().JsonReg(id_doc);
                    if (new RelatorRN().Excluir(id_doc))
                    {
                        ...
                        try
                        {
                            ...
                            excluidoOv.json_doc_excluido = relatorOv;
                            excluidoOv.nm_base_excluido = Config.ValorChave("NmBaseRelator", true);
