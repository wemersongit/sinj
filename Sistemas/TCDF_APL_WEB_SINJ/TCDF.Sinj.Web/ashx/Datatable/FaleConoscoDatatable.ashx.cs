...
{
    /// <summary>
    /// Summary description for FaleConoscoDatatable
    /// </summary>
    public class FaleConoscoDatatable : IHttpHandler
    {
        ...
        public void ProcessRequest(HttpContext context)
        {
            ...
            try
            {
                ...
                {
                    query += (!string.IsNullOrEmpty(query) ? " AND " : "") + "Upper(document::text) like '%" + _sSearch.ToUpper() + "%'";
                }
                pesquisa.literal = query;
                json_resultado = new FaleConoscoRN().JsonReg(pesquisa);
                ...
                LogOperacao.gravar_operacao("FLC.PES", Busca, sessao_usuario.nm_usuario, sessao_usuario.nm_login_usuario);
            }
            catch (Exception ex)
            {
                ...
                {
                    LogErro.gravar_erro("FLC.PES", erro, sessao_usuario.nm_usuario, sessao_usuario.nm_login_usuario);
                }
            }
